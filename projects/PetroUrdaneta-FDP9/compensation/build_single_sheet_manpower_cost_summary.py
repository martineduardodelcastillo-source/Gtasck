...
def build():
    ...
    ws["C8"] = (
        "BIG NOTE — Alexander Stulme, Alan McKeon and Martin Aguero are secondees. The model deducts an indicative 60% PU salary-share reimbursement only, pending Barbara's confirmation of eligibility and any cap. "
        "Martin is already assigned to the project as a local secondee. Repatriates receive the one-time transition package in M7 and then become local. Alan remains the only expat and receives $2,000/month housing from M7."
    )
    ...
    summary_labels = ["Gross manpower cost", "Less: PU salary share @60%", "NET manpower cost"]
    for row, label in enumerate(summary_labels, start=13):
        ...
        if row == 13:
            ...
            formulas = [
                f'=-SUMIF($G${TABLE_START}:$G${TABLE_END},"60%",$L${TABLE_START}:$L${TABLE_END})*60%',
                f'=-SUMIF($G${TABLE_START}:$G${TABLE_END},"60%",$L${TABLE_START}:$L${TABLE_END})*60%',
                f'=-SUMIF($G${TABLE_START}:$G${TABLE_END},"60%",$L${TABLE_START}:$L${TABLE_END})*60%',
                f'=-SUMIF($G${TABLE_START}:$G${TABLE_END},"60%",$I${TABLE_START}:$I${TABLE_END})*60%',
            ]
        else:
    ...
    ws["I12"] = (
        ...
        "For M1–M6, the eight transition roles are assumed to work 21 days on / 14 days off: 1.71 tickets per person per month, or 13.71 tickets per month in total. Ticket costs are excluded pending Airswift confirmation. Barbara must confirm the 60% secondee salary-share reimbursement and any cap."
    )
    ...
    headers = [
        "Person", "Position", "M1–M6 Contract", "M7+ Basis", "PU Salary Share", "Tickets / Mo M1–M6", "Annual Salary",
    ...
    for idx, record in enumerate(PEOPLE):
        ...
        pu_salary_share = "60%" if pu_paid == "Yes" else "—"
        set_input(ws.cell(row, 7), pu_salary_share, management_note("Working assumption: PU reimbursement equals 60% of the relevant secondee salary only. Barbara must confirm eligibility, documentation requirements and any reimbursement cap."))
        set_formula(
            ws.cell(row, 8),
...
def validate():
    ...
    assert ws["D14"].value == '=-SUMIF($G$21:$G$29,"60%",$L$21:$L$29)*60%'
    assert ws["E13"].value == "=SUM(M21:M29)"
    assert ws["G13"].value == "=SUM(O21:O29)"
    assert ws["G14"].value == '=-SUMIF($G$21:$G$29,"60%",$I$21:$I$29)*60%'
    assert ws["G15"].value == "=G13+G14"
    print("VALIDATED: one-sheet workbook | 60% secondee salary share pending Barbara | costs recalculated")
